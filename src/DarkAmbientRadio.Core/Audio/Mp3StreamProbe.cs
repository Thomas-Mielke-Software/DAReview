...
public static class Mp3StreamProbe
{
    ...
    public static Mp3StreamInfo? Probe(string filePath)
    {
        try
        {
            ...
            while (stream.Read(header, 0, 4) == 4)
            {
                ...
                if (frameIndex == 0 && HasVbrTag(stream, frame))
                {
                    frameIndex++;
    ...
    private readonly record struct FrameHeader(int Bitrate, int Length, bool IsMpeg1, bool IsMono);
    ...
    private static bool TryParseHeader(ReadOnlySpan<byte> h, out FrameHeader frame)
    {
        ...
        frame = new FrameHeader(bitrate, length, isMpeg1, IsMono: ((h[3] >> 6) & 0x03) == 3);
        return true;
    }

    /// <summary>
    /// Peeks whether this frame carries a Xing/Info/VBRI header, without moving the stream.
    /// The tag sits right behind the side information, whose size depends on version and mode.
    /// </summary>
    private static bool HasVbrTag(FileStream stream, FrameHeader frame)
    {
        ...
        try
        {
            ...
            if (stream.Read(tag) == 4 && (Is(tag, "Xing") || Is(tag, "Info")))
                return true;

            // VBRI (Fraunhofer) always sits 32 bytes behind the header, regardless of mode.
            stream.Position = resume + 32;
            return stream.Read(tag) == 4 && Is(tag, "VBRI");
        }
        finally
        {
    ...
    private static bool TrySync(FileStream stream, long start, out long position)
    {
        ...
        for (var i = 0; i + 4 <= read; i++)
        {
            ...
            if (TryParseHeader(buffer.AsSpan(i, 4), out _))
            {
                position = start + i;
                return true;
            }
